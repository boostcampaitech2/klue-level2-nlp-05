...
def main(dataset:pd.DataFrame, aug) -> pd.DataFrame:
    '''
...
def iterate_main(path:str, min_num=300):
    '''
    data imbalance를 해소하기 위해 특정 라벨 데이터를 최소 수준까지 증강
    '''
    df = pd.read_csv(path)
    ...
    df.to_csv(f"/opt/ml/dataset/aeda_bal{min_num}_dataset/train/train.csv", header=True, index=False)
