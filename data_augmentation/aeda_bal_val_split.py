...
def iterate_main(path, min_num=300):
    ...
    final_df_train.to_csv(f"{DATASET_PATH}/aeda_bal{min_num}_dataset/train/train.csv", header=True, index=False)
    final_df_val.to_csv(f"{DATASET_PATH}/aeda_bal{min_num}_dataset/train/valid.csv", header=True, index=False)
